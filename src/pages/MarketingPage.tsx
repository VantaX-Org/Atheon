...
.mk-glass{background:rgba(255,255,255,.7);backdrop-filter:blur(20px);-webkit-backdrop-filter:blur(20px);border:1px solid rgba(0,0,0,.06);}
.mk-glass:hover{background:rgba(255,255,255,.9);border-color:rgba(99,102,241,.15);}
.mk-glass-strong{background:rgba(255,255,255,.8);backdrop-filter:blur(30px);-webkit-backdrop-filter:blur(30px);border:1px solid rgba(0,0,0,.06);}
.mk-bento-tilt{transition:transform .6s cubic-bezier(.16,1,.3,1),box-shadow .6s ease;}
.mk-bento-tilt:hover{transform:translateY(-6px) perspective(800px) rotateX(2deg);box-shadow:0 20px 50px rgba(0,0,0,.08),0 0 30px rgba(99,102,241,.06);}
`;
...
const securityFeatures = [
  { label: "AES-256 Encryption at Rest", Icon: IconShield },
  { label: "Azure AD SSO Integration", Icon: IconControlPlane },
  { label: "Full Audit Trail Logging", Icon: IconAudit },
  { label: "Tenant Data Isolation (D1)", Icon: IconNetwork },
  { label: "RBAC & Role-Based Access", Icon: IconControlPlane },
  { label: "PBKDF2 Password Hashing", Icon: IconShield },
  { label: "GDPR & POPIA Aligned", Icon: IconCheckCircle },
  { label: "Cloudflare Edge Security", Icon: IconConnectivity },
];
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (const p of pts) {
        ...
        ctx.fillStyle = `rgba(99, 102, 241, ${p.o * 0.6})`;
        ctx.fill();
      }
...
export function MarketingPage() {
  ...
  return (
    ...
    >
      ...
      {/* ========== NAVBAR ========== */}
      <nav className="fixed top-0 w-full z-50" style={{ background: "rgba(255, 255, 255, 0.85)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <a href="#" className="flex items-center gap-2.5 group">
            ...
            <span className="text-base font-bold tracking-tight text-slate-900">Atheon</span>
          </a>
          <div className="hidden md:flex items-center gap-8 text-[13px] font-medium text-slate-500">
            <a href="#platform" className="hover:text-slate-900 transition-colors duration-200">Platform</a>
            <a href="#catalysts" className="hover:text-slate-900 transition-colors duration-200">Catalysts</a>
            <a href="#how" className="hover:text-slate-900 transition-colors duration-200">How It Works</a>
            <a href="#security" className="hover:text-slate-900 transition-colors duration-200">Security</a>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate("/login")} className="text-[13px] font-medium px-4 py-2 rounded-lg text-slate-600 hover:text-slate-900 hover:bg-slate-100 transition-all duration-200">Sign In</button>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
        ...
        {/* Gradient orbs */}
        <div className="absolute top-1/4 left-1/4 w-[700px] h-[700px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(99, 102, 241, 0.08) 0%, transparent 70%)", animation: "mk-glow-breathe 8s ease-in-out infinite" }} />
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(139, 92, 246, 0.06) 0%, transparent 70%)", animation: "mk-glow-breathe 10s ease-in-out 3s infinite" }} />
        <div className="absolute top-1/3 right-1/6 w-[300px] h-[300px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(14, 165, 233, 0.04) 0%, transparent 70%)", animation: "mk-glow-breathe 7s ease-in-out 1s infinite" }} />
        ...
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(600px circle at var(--mx) var(--my), rgba(99, 102, 241, 0.03), transparent 60%)" }} />

        {/* Hero content */}
        <div className="relative max-w-5xl mx-auto px-6 text-center z-10">
          <div style={{ animation: "mk-hero-text 1s cubic-bezier(.16,1,.3,1) forwards", animationDelay: "0.1s", opacity: 0 }}>
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[11px] font-semibold mb-10 border border-indigo-200" style={{ background: "rgba(99, 102, 241, 0.06)", color: "#4f46e5" }}>
          ...
          <h1 style={{ animation: "mk-hero-text 1s cubic-bezier(.16,1,.3,1) forwards", animationDelay: "0.25s", opacity: 0 }} className="text-5xl sm:text-6xl lg:text-[5.5rem] font-extrabold leading-[1.02] tracking-tight mb-8">
            <span className="text-slate-900">The AI that doesn{"\u2019"}t</span><br />
            <span className="text-slate-900">just analyse </span>
            <span style={{ backgroundImage: "linear-gradient(135deg, #4f46e5, #6366f1, #7c3aed, #0ea5e9, #4f46e5)", backgroundSize: "300% auto", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "mk-text-shimmer 5s linear infinite" }}>
              {"— it acts"}
            </span>
          </h1>

          <p style={{ animation: "mk-hero-text 1s cubic-bezier(.16,1,.3,1) forwards", animationDelay: "0.45s", opacity: 0 }} className="text-lg lg:text-xl text-slate-500 leading-relaxed max-w-2xl mx-auto mb-4">
          ...
          <p style={{ animation: "mk-hero-text 1s cubic-bezier(.16,1,.3,1) forwards", animationDelay: "0.55s", opacity: 0 }} className="text-sm text-slate-400 max-w-xl mx-auto mb-12">
          ...
          <div style={{ animation: "mk-hero-text 1.2s cubic-bezier(.16,1,.3,1) forwards", animationDelay: "0.85s", opacity: 0 }} className="grid grid-cols-2 sm:grid-cols-4 gap-6 max-w-3xl mx-auto">
            {stats.map((s) => (
              <div key={s.label} className="text-center group">
                <div className="text-3xl lg:text-4xl font-extrabold text-slate-900 mb-1 transition-all duration-300 group-hover:text-indigo-600" style={{ animation: "mk-number-pop 4s ease-in-out infinite" }}>
                  <AnimatedCounter value={s.value} prefix={s.prefix} suffix={s.suffix} />
                </div>
                <div className="text-[10px] font-semibold uppercase tracking-[0.15em] text-slate-400">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2" style={{ animation: "mk-float 2s ease-in-out infinite" }}>
          <div className="w-6 h-10 rounded-full border-2 border-slate-200 flex items-start justify-center p-1.5">
            <div className="w-1 h-2.5 rounded-full bg-slate-300" style={{ animation: "mk-float 1.5s ease-in-out infinite" }} />
          </div>
        </div>
      </section>
      ...
      <section id="platform" className="py-24 lg:py-36 relative">
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 50% at 50% 0%, rgba(99, 102, 241, 0.03) 0%, transparent 60%)" }} />
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <div className="mk-reveal inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-semibold mb-5 uppercase tracking-[0.15em] border border-indigo-200" style={{ background: "rgba(99, 102, 241, 0.05)", color: "#4f46e5" }}>
              Platform Architecture
            </div>
            <h2 className="mk-reveal mk-reveal-d1 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight mb-5">
              Six layers of <span style={{ backgroundImage: "linear-gradient(135deg, #4f46e5, #6366f1)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>intelligence</span>
            </h2>
            <p className="mk-reveal mk-reveal-d2 text-base text-slate-500 max-w-xl mx-auto leading-relaxed">
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-[13px] text-slate-500 leading-relaxed mb-5">{layer.desc}</p>
      ...
      <section id="catalysts" className="py-24 lg:py-36 relative overflow-hidden">
        {/* Animated rings */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[700px] rounded-full pointer-events-none" style={{ border: "1px solid rgba(139, 92, 246, 0.08)", animation: "mk-pulse-ring 5s ease-in-out infinite" }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full pointer-events-none" style={{ border: "1px solid rgba(99, 102, 241, 0.06)", animation: "mk-pulse-ring 5s ease-in-out 1.5s infinite" }} />

        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 60% 40% at 50% 50%, rgba(139, 92, 246, 0.03) 0%, transparent 60%)" }} />

        <div className="relative max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <div className="mk-reveal inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-[11px] font-bold mb-6 uppercase tracking-[0.15em] border" style={{ background: "rgba(139, 92, 246, 0.06)", color: "#7c3aed", borderColor: "rgba(139, 92, 246, 0.2)", animation: "mk-border-glow 3s ease-in-out infinite" }}>
              <IconCatalysts size={14} /> The Evolution of AI Agents
            </div>
            <h2 className="mk-reveal mk-reveal-d1 text-3xl sm:text-4xl lg:text-[3.5rem] font-extrabold text-slate-900 tracking-tight mb-6 leading-[1.05]">
              Meet{" "}
              <span style={{ backgroundImage: "linear-gradient(135deg, #7c3aed, #8b5cf6, #6366f1, #7c3aed)", backgroundSize: "300% auto", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", animation: "mk-text-shimmer 4s linear infinite" }}>
                Catalysts
              </span>
            </h2>
            <p className="mk-reveal mk-reveal-d2 text-base text-slate-500 max-w-2xl mx-auto leading-relaxed">
              Today{"\u2019"}s AI assistants tell you what to do. <strong className="text-slate-700">Catalysts actually do it.</strong> Purpose-built autonomous agents that understand context, execute workflows, handle exceptions, and learn.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-16">
            ...
            <div className="mk-reveal mk-reveal-d1 mk-glass rounded-2xl p-6">
              <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 mb-4">Traditional RPA</div>
            ...
            <div className="mk-reveal mk-reveal-d2 mk-glass rounded-2xl p-6">
              <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-400 mb-4">AI Copilots</div>
              <div className="space-y-3">
                {["Recommendations only", "Human must execute", "Limited domain knowledge", "No persistent memory"].map(item => (
                  <div key={item} className="flex items-center gap-2.5 text-[13px] text-slate-400">
                    <IconCross size={12} className="text-amber-500 flex-shrink-0" /> {item}
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Atheon Catalysts */}
            <div className="mk-reveal mk-reveal-d3 rounded-2xl p-6 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(139, 92, 246, 0.05), rgba(99, 102, 241, 0.03))", border: "1px solid rgba(139, 92, 246, 0.2)", animation: "mk-border-glow 3s ease-in-out infinite" }}>
              ...
              <div className="text-[10px] font-bold uppercase tracking-[0.15em] text-violet-600 mb-4">Atheon Catalysts</div>
              <div className="space-y-3">
                {["Autonomous execution", "Handles exceptions", "Full domain context", "Learns & improves"].map(item => (
                  <div key={item} className="flex items-center gap-2.5 text-[13px] text-slate-800 font-medium">
              ...
              return (
                <div key={uc.title} className={`mk-reveal mk-reveal-d${i + 1} mk-glass mk-bento-tilt rounded-2xl p-6 group`}>
                  <div className="flex items-start gap-5">
                    <div className="w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-500 group-hover:scale-110" style={{ background: "rgba(139, 92, 246, 0.06)", boxShadow: "0 0 0 1px rgba(139, 92, 246, 0.12)" }}>
                      <UCIcon size={24} style={{ color: "#7c3aed" }} />
                    </div>
                    <div className="flex-1">
                      <h3 className="text-sm font-bold text-slate-900 mb-1.5">{uc.title}</h3>
                      <p className="text-[13px] text-slate-500 leading-relaxed mb-4">{uc.desc}</p>
                      <div className="flex items-baseline gap-2">
                        <span className="text-2xl font-extrabold text-violet-600">{uc.metric}</span>
                        <span className="text-[11px] font-medium text-slate-400">{uc.metricLabel}</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      {/* ========== WHY ATHEON ========== */}
      <section className="py-24 lg:py-36 relative" style={{ background: "linear-gradient(180deg, #f8fafc 0%, #ffffff 100%)" }}>
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 70% 40% at 50% 100%, rgba(99, 102, 241, 0.03) 0%, transparent 60%)" }} />
        <div className="relative max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <div className="mk-reveal inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-semibold mb-5 uppercase tracking-[0.15em] border border-indigo-200" style={{ background: "rgba(99, 102, 241, 0.05)", color: "#4f46e5" }}>
              Why Atheon
            ...
            {whyAtheon.map((item, i) => {
              ...
              return (
                <div key={item.title} className={`mk-reveal mk-reveal-d${i + 1} mk-glass mk-bento-tilt rounded-2xl p-7 group`}>
                  <div className="flex items-start gap-5">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-500 group-hover:scale-110 group-hover:rotate-3" style={{ background: "rgba(99, 102, 241, 0.05)", boxShadow: "0 0 0 1px rgba(99, 102, 241, 0.1)" }}>
                      <WIcon size={22} className="text-indigo-600" />
                    </div>
                    <div>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <h2 className="mk-reveal mk-reveal-d1 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight mb-5">
              ...
              <span style={{ backgroundImage: "linear-gradient(135deg, #4f46e5, #6366f1)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>seconds</span>
            </h2>
            <p className="mk-reveal mk-reveal-d2 text-base text-slate-500 max-w-lg mx-auto leading-relaxed">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 relative">
            ...
            <div className="hidden lg:block absolute top-20 left-[15%] right-[15%] h-px">
              <div className="h-full w-full" style={{ background: "linear-gradient(90deg, transparent, rgba(99, 102, 241, 0.12), rgba(99, 102, 241, 0.12), transparent)" }} />
            ...
            {steps.map((s, i) => {
              ...
              return (
                <div key={s.num} className={`mk-reveal mk-reveal-d${i + 1} mk-glass mk-bento-tilt rounded-2xl p-6 text-center group`}>
                  <div className="w-16 h-16 rounded-2xl mx-auto mb-6 flex items-center justify-center transition-all duration-500 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-indigo-500/15" style={{ background: "rgba(99, 102, 241, 0.05)", boxShadow: "0 0 0 1px rgba(99, 102, 241, 0.1)" }}>
                    <SIcon size={26} className="text-indigo-600" />
                  </div>
                  <div className="text-[10px] font-bold mb-3 uppercase tracking-[0.2em] text-indigo-300">{s.num}</div>
      ...
      {/* ========== SECURITY ========== */}
      <section id="security" className="py-24 lg:py-36 relative" style={{ background: "linear-gradient(180deg, #f8fafc 0%, #f1f5f9 100%)" }}>
        <div className="max-w-5xl mx-auto px-6">
          <div className="mk-reveal mk-glass-strong rounded-3xl p-10 lg:p-16 relative overflow-hidden">
            {/* Background glow */}
            <div className="absolute top-0 right-0 w-80 h-80 rounded-full pointer-events-none" style={{ background: "radial-gradient(circle, rgba(99, 102, 241, 0.04) 0%, transparent 70%)", animation: "mk-glow-breathe 6s ease-in-out infinite" }} />

            <div className="flex flex-col lg:flex-row items-start gap-12 relative">
              <div className="flex-1">
                <div className="w-14 h-14 rounded-xl flex items-center justify-center mb-6" style={{ background: "rgba(99, 102, 241, 0.06)", boxShadow: "0 0 0 1px rgba(99, 102, 241, 0.1)", animation: "mk-float-slow 5s ease-in-out infinite" }}>
                  <IconShield size={28} className="text-indigo-600" />
                </div>
                <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight mb-5">Enterprise-grade<br />security</h2>
                <p className="text-[14px] text-slate-500 leading-relaxed mb-4 max-w-md">
                  Role-based access control. AES-256 encryption at rest. Comprehensive audit logging across every action. Tenant-isolated data on Cloudflare D1.
                </p>
              ...
              <div className="flex-1 w-full">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {securityFeatures.map((f, i) => {
                    const FIcon = f.Icon;
                    return (
                      <div key={f.label} className={`mk-reveal mk-reveal-d${(i % 4) + 1} flex items-center gap-3 p-3.5 rounded-xl transition-all duration-300 hover:bg-indigo-50/50`}>
                        <FIcon size={16} className="text-indigo-600 flex-shrink-0" />
                        <span className="text-[13px] font-medium text-slate-600">{f.label}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      <section className="py-28 lg:py-40 relative overflow-hidden">
        {/* Gradient background */}
        <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 60% at 50% 50%, rgba(99, 102, 241, 0.04) 0%, transparent 60%)" }} />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] rounded-full pointer-events-none" style={{ border: "1px solid rgba(99, 102, 241, 0.06)", animation: "mk-pulse-ring 6s ease-in-out infinite" }} />
        ...
        <div className="relative max-w-3xl mx-auto px-6 text-center">
          ...
          <h2 className="mk-reveal mk-reveal-d1 text-3xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight mb-6">
            Ready to redefine<br />enterprise intelligence?
          </h2>
          <p className="mk-reveal mk-reveal-d2 text-base text-slate-500 leading-relaxed mb-4 max-w-lg mx-auto">
            Join the organisations deploying Catalysts to transform operational data into autonomous action.
          </p>
          <p className="mk-reveal mk-reveal-d3 text-sm text-slate-400 leading-relaxed mb-12 max-w-md mx-auto">
          ...
          <div className="mk-reveal mk-reveal-d4 flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <button className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-[15px] font-semibold text-slate-600 border border-slate-200 transition-all duration-300 hover:bg-slate-50 hover:text-slate-900 hover:border-slate-300">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm font-bold tracking-tight text-slate-700">Atheon</span>
            </div>
            <div className="flex items-center gap-8 text-[13px] text-slate-400">
              <a href="#platform" className="hover:text-slate-700 transition-colors">Platform</a>
              <a href="#catalysts" className="hover:text-slate-700 transition-colors">Catalysts</a>
