  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br        from-[#2a7c8c]/20 to-[#2a7c8c]/10 flex items-center justify-center">
                  <MessageSquare className="w-5 h-5 text-[#2a7c8c]"/>
        </div>
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="space-y-3">
          ...
          <Card hover className="border-[#2a7c8c]/20">
            ...
            <div className="flex-1 space-y-4 mb-4 overflow-y-auto">
              ...
              {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : ''}`}>
                  {msg.role === 'assistant' && (
                    <div className="w-8 h-8 rounded-lg                                      bg-gradient-to-br from-[#2a7c8c] to-[#1a5c68] flex items-center justify-center flex-shrink-0">
                                                            <Sparkles className="w-4 h-4 text-white" />
                                      </div>
                                    )}
                                    <div className={`max-w-2xl rounded-xl p-4 ${
                                      msg.role === 'user'
                                        ? 'bg-[#2a7c8c]/100/10 border border-[#2a7c8c]/20'
                      ...
                      <div className="mt-3 space-y-1.5">
                        {msg.citations.map((cit) => (
                          <div key={cit.id}                          className="flex items-center gap-2 p-2 rounded bg-white/[0.04] border border-white/[0.06] text-xs">
                                                      <span className="text-[#2a7c8c]">📎</span>
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 relative">
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm text-white placeholder-gray-400 backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-[#2a7c8c]/20 focus:border-[#2a7c8c]/30 transition-all"
                />
              </div>
