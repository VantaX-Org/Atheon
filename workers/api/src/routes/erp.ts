...
erp.get('/tcp-debug', async (c) => {
  ...
  const mode = c.req.query('mode') || 'no-close'; // 'close' or 'no-close'
  const steps: string[] = [];
  ...
  try {
    ...
    steps.push('2. connect() called with allowHalfOpen: true');
    ...
    if (mode === 'close') {
      ...
      writer.releaseLock();
      steps.push('5. writer.releaseLock() called (NOT closed)');
    }
    ...
    return c.json({ success: true, mode, steps });
  } catch (err) {
    steps.push(`ERROR: ${err}`);
    return c.json({ success: false, mode, steps, error: String(err) });
  }
});
