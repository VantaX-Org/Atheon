export async function seedDatabase(db: D1Database) {
  ...
  // Check if tenants were intentionally deleted — don't re-seed if so
  try {
    const deleted = await db.prepare('SELECT COUNT(*) as count FROM deleted_tenants').first<{ count: number }>();
    if (deleted && deleted.count > 0) return;
  ...
  const adapters = [
    ...
    { id: 'erp-d365', name: 'Microsoft Dynamics 365', system: 'Dynamics365', version: '10.0.42', protocol: 'OData v4', status: 'available', operations: '["OData","Custom API","Power Automate","Dataverse"]', auth_methods: '["Azure AD OAuth","Service Principal"]' },
    ...
    { id: 'erp-ns', name: 'NetSuite', system: 'NetSuite', version: '2026.1', protocol: 'REST/SuiteTalk', status: 'available', operations: '["REST API","SuiteTalk SOAP","SuiteQL","RESTlets"]', auth_methods: '["OAuth 2.0","Token-Based Auth"]' },
